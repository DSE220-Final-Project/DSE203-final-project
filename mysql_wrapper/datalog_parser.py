...
class DatalogParser(object):
  ...
  def print_output(self, select,from_table,where_equality,where_inequality,predicates_constraint_dict,groupby_vars,having_dict,sort_var,sort_type,limit_num):
  ...
  def parse(self, datalog_str):
    ...
    if len(re.findall(separator,datalog_str))>1:
        ...
        for i in range(len(right_all)):
            output.append(parse(left,right_all[i]))
        ...
        if all(x == equalities[0] for x in equalities) and all(x == inequalities[0] for x in inequalities):
            print 1
            sql_query=print_output(output[0][0],output[0][1],output[0][2],output[0][3],output[0][4],output[0][5],output[0][6],output[0][7],output[0][8],output[0][9])
        ...
        ## When there are different criteria in the inequalities => or in inequalities
            ...
            sql_query=print_output(output[0][0],output[0][1],output[0][2],output[0][3],all_inequalities_new_dict,output[0][5],output[0][6],output[0][7],output[0][8],output[0][9])
        ...
        ## When there are different criteria in the equalities => or in equalities
            ...
            sql_query= print_output(output[0][0],output[0][1],all_equalities_new_dict,output[0][3],output[0][4],output[0][5],output[0][6],output[0][7],output[0][8],output[0][9])
        ...
        ## When there are different criteria in both equalities and inequalities => or in both equalities and inequalities
            ...
            for k,v in [(key,d[key]) for d in all_equalities for key in d]:
                if k not in all_equalities_new_dict: 
                    ...
                    all_equalities_new_dict[k].append(v)
            sql_query=print_output(output[0][0],output[0][1],all_equalities_new_dict,output[0][3],all_inequalities_new_dict,output[0][5],output[0][6],output[0][7],output[0][8],output[0][9])

    ## For non OR query
    else:
        left = datalog_str.split(separator )[0]
        right = datalog_str.split(separator )[1]
        output = self._parse(left,right)
        sql_query=self.print_output(output[0],output[1],output[2],output[3],output[4],output[5],output[6],output[7],output[8],output[9])
